...
bids_path=/mnt/chrastil/data2/users/liz/MLINDIV3/bids_data/BIDS_data

code_path=${bids_path}/code #this is where you should put both scripts. BIDS standard.


for i in `seq 0 ${no_of_subjects}`;
do
	qsub -o ${bids_path}/gridlog/fmriprep_${subjects[i]}.txt ${code_path}/run_fmriprep.sh ${subjects[i]} #the -o and -e tags just send the output and error messages to convenient folders.
done
